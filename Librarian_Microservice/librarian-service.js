...
app.post('/addbook', (req, res) => {
    ...
    book.save()
        .then(saved => res.status(200).json({ message: 'Book added', book: saved }))
        .catch(err => res.status(500).json({ message: err.message || 'Error adding book' }));
});
...
app.get('/updatebook', (req, res) => {
    BookModel.findOneAndUpdate(
        ...
        .catch(err => res.status(500).json({ message: err.message || 'Error updating book' }));
});
...
app.delete('/deletebook', (req, res) => {
    BookModel.findOneAndDelete({ bookid: req.body.bookid })
        ...
        .catch(err => res.status(500).json({ message: err.message || 'Error deleting book' }));
});
